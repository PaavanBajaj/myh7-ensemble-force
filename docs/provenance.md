# Provenance

How this public repository cites evidence. Vault evidence is **cite, do not
contain**: literature PDFs, full-text extracts, restricted supplements, and
raw working data stay in the encrypted private data vault. Public materials
point at scrubbed identifiers, checksums, and freeze metadata only.

## Status (S0)

Provenance account stub only. Schema and manifest stubs for the LSAR slice
arrive at S1 under `data/public/manifests/`.

## Rules

1. Cite sources by stable IDs and checksums published in public manifests.
2. Do not commit copyrighted full text, screenshots of papers, or vault paths
   that imply redistribution.
3. Scrubbed shareable derivatives (when any exist) live under `data/public/`
   only after eligibility and copyright review.
